...
permitted_ips = ['1.1.1.1', '127.0.0.1']
...
class MainHandler(webapp2.RequestHandler):
    ...
    def post(self):
        try:
            ...
            try:
                i = permitted_ips.index(ip)
            except ValueError:
                logging.error('Unauthorized access from %s' % ip)
                logging.info('REMOTE ADDR: %s' % ip)
                return # no match
...
application = webapp2.WSGIApplication([("/fetch.py", MainHandler)])
